...
//    std::cout << it->xid <<", cb_present = "<<it->cb_present << std::endl;
  ...
  //delete obsolete rpc, XIDs <= xid_rep
/*   it = reply_window_[clt_nonce].begin();
   while (it != reply_window_[clt_nonce].end() && it->xid <= xid_rep && it->cb_present == true)  
   {
	//frees the reply_t::buf; 
	 std::cout << "erase: xid = " << it->xid << std::endl;
  ...
  if(it != reply_window_[clt_nonce].end() 
      && xid < it -> xid ) // no the third condition
  { 
	std::cout << "=========return forgetten========" << std::endl;
    return FORGOTTEN;
  }
*/
